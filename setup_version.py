...
# Path to file with version info accessible by normetapi:
...
def generate_version_string(version):
    """Generate a string with the current version.

    Parameters
    ----------
    version : dict
        A dict containing the current version.
    ...
    if pathlib.Path('.git').is_dir():
        ...
        try:
            ...
            msg = (
                ...
                f'{VERSION_FILE} and the build directory '
                'before building.'
            )
...
def write_version_py(version):
    """Create a file with the version info.
